...
public class KafkaSteps extends ScenarioSteps {
  ...
  public void prepareMessage(String variable, String desc, String value) {
    ...
    Serenity.recordReportData().withTitle("Kafka message that we want to produce").
        andContents(kafkaData.getModel());
  }
  ...
  public void validateSystemSuccessToListenRelatedMessage(String condition)
    ...
    if (condition.equalsIgnoreCase("message")) {
      ...
      kafkaMessage = kafkaUtility.findMessageByKey(kafkaData.getConsumer(), kafkaData.getKey());
    }
    Serenity.recordReportData().withTitle("Kafka Message that we consume").andContents(kafkaMessage);
    assertThat("kafka message is not match",
        kafkaMessage, equalTo(kafkaData.getModel()));
  }
}
